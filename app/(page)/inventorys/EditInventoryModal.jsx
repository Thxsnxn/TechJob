...
import {
    ClipboardList, X, FileText, Hash, User, Building, Package, CalendarDays, Pencil, Plus, Trash2,
} from "lucide-react";
...
import { Select, SelectTrigger, SelectValue, SelectContent, SelectItem } from "@/components/ui/select"; // เพิ่ม Select
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";


export default function EditInventoryModal({
    onClose, onSubmit, inventoryData, stockData, findStockInfo
}) {
    ...
    useEffect(() => {
        if (inventoryData) {
            const formattedOrderDate = inventoryData.orderDate ? inventoryData.orderDate.split("/").reverse().join("-") : "";
            const formattedDeliveryDate = inventoryData.deliveryDate ? inventoryData.deliveryDate.split("/").reverse().join("-") : "";

            setId(inventoryData.id || ""); setSupplier(inventoryData.supplier || ""); setOrderbookId(inventoryData.orderbookId || "");
            setOrderDate(formattedOrderDate); setDeliveryDate(formattedDeliveryDate);
            setRequester(inventoryData.details?.requester || ""); setDepartment(inventoryData.details?.department || "");
    ...
    const handleItemChange = (index, field, value) => {
        ...
        if (field === 'itemCode') {
            ...
            if (stockItem) {
                newItems[index].itemName = stockItem.itemName; newItems[index].unit = stockItem.unit;
                newItems[index].unitPkg = stockItem.unitPkg; newItems[index].packSize = String(stockItem.packSize);
                newItems[index].vendorItemCode = stockItem.supplierName || "N/A";
                newItems[index].itemNameVendor = stockItem.itemName; newItems[index].itemNameDetail = stockItem.itemName;
            } else {
    ...
    const handleRemoveItem = (index) => {
        ...
        if (!window.confirm(`คุณแน่ใจหรือไม่ว่าต้องการลบรายการอะไหล่: ${itemToRemove.itemName || itemToRemove.itemCode} ออกจากใบเบิกนี้?`)) { return; }
    ...
    const handleSubmit = () => {
        ...
        for (const item of items) {
            ...
            if (stockInfo && requestedQty > stockInfo.stock) {
                alert(`ไม่สามารถบันทึกได้: รายการ ${item.itemCode} (จำนวน ${requestedQty} ${item.unit || 'ชิ้น'}) เกิน Stock ที่มีอยู่ (${stockInfo.stock} ${stockInfo.unit})`);
                hasStockIssue = true;
                break;
            }
        }
        ...
        if (!window.confirm(`คุณแน่ใจหรือไม่ว่าต้องการบันทึกการแก้ไขใบเบิก ${inventoryData.orderbookId} นี้?`)) { return; }

        const editedInventoryOrder = {
            ...inventoryData, id, supplier, orderbookId, orderDate: format(new Date(orderDate), "dd/MM/yyyy"),
            ...
            details: { ...inventoryData.details, requester: requester, department: department, lastEditor: "ผู้ใช้ปัจจุบัน (แก้ไข)", lastEditDate: format(new Date(), "dd/MM/yyyy HH:mm:ss"), },
        ...
        <>
            ...
            <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 
                ...
                <CardHeader className="flex flex-row items-center justify-between border-b px-6 py-4 bg-gradient-to-r from-yellow-500 to-yellow-700 text-white">
                    <h2 className="text-2xl font-bold flex items-center gap-2"><Pencil className="h-7 w-7" />แก้ไขรายการเบิก ( {inventoryData.orderbookId} )</h2>
                    <Button variant="ghost" size="icon" onClick={onClose} className="text-white hover:bg-yellow-600"><X className="h-6 w-6" /></Button>
                </CardHeader>
                ...
                <CardContent className="p-6 space-y-6 overflow-y-auto flex-grow bg-gray-50 dark:bg-gray-800">
                    <Card className="shadow-md">
                        ...
                        <CardContent className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            <div className="space-y-2"><Label htmlFor="orderbookId" className="flex items-center gap-1 text-gray-700 dark:text-gray-300"><FileText className="h-4 w-4 text-gray-500" /> เลขที่เอกสาร</Label><Input id="orderbookId" placeholder="ระบุเลขที่เอกสาร" value={orderbookId} onChange={(e) => setOrderbookId(e.target.value)} disabled={isOrderbookIdDisabled}/></div>
                    ...
                    <Card className="shadow-md">
                        <CardHeader className="bg-purple-50 dark:bg-purple-900 border-b pb-4 flex flex-row items-center justify-between">
                            <div className="flex items-center gap-2"><Package className="h-5 w-5 text-purple-600 dark:text-purple-300" /><h3 className="text-lg font-semibold text-purple-700 dark:text-purple-200">รายการอะไหล่/วัสดุ</h3></div>
                            ...
                            <Table className="min-w-full">
                                ...
                                <TableBody>
                                    {items.length > 0 ? (
                                            ...
                                            return (
                                                <TableRow key={index} className={isQtyExceeded ? "bg-red-50" : ""}>
                                                    ...
                                                    <TableCell className="w-[150px]">
                                                        {/* ** เปลี่ยนเป็น Select Box เหมือน Create Modal ** */}
                                                        <Select
                                                            onValueChange={(value) => handleItemChange(index, 'itemCode', value)}
                                                            value={item.itemCode}
                                                        >
                                                            <SelectTrigger>
                                                                <SelectValue placeholder={item.itemCode || "-- เลือกรหัสอะไหล่ --"} />
                                                            </SelectTrigger>
                                                            <SelectContent>
                                                                {stockData.map((s) => (
                                        ...
                                        <TableRow>
                                            <TableCell colSpan={7} className="text-center text-muted-foreground h-16">ไม่พบรายการอะไหล่ โปรดเพิ่มรายการ</TableCell>
                                        </TableRow>
                                    )}
                                </TableBody>
                            </Table>
                ...
                <CardFooter className="border-t p-4 flex justify-end gap-3 bg-white dark:bg-gray-900">
                    ...
                    <Button className="bg-yellow-600 hover:bg-yellow-700" onClick={handleSubmit}>
                        <Pencil className="mr-2 h-4 w-4" />บันทึกการแก้ไข
                    </Button>
                </CardFooter>
            </div>
        </>
