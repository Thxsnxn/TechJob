...
export default function AddEquipmentModal({
  ...
  // --- [แก้ไข] เพิ่มการรับค่า maxVal เพื่อล็อคจำนวน ---
  const handleQuantityChange = (itemId, qty, maxVal) => {
    let newQty = qty;

    // เช็คกรณีค่าว่างหรือ NaN ให้เป็น 1
    if (!newQty || isNaN(newQty) || newQty < 1) newQty = 1;

    // เช็คห้ามเกินจำนวนที่มี (maxVal)
    if (newQty > maxVal) {
      newQty = maxVal;
    }
  ...
  return (
    <>
              ...
              <table className="w-full text-sm text-left min-w-[900px]">
                <thead className="bg-gray-100 text-gray-700 dark:bg-slate-800 dark:text-slate-300 sticky top-0 z-10 shadow-sm">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-center text-blue-600">
                      คงเหลือ
                    </th>
                ...
                <tbody className="divide-y dark:divide-slate-800 bg-white dark:bg-slate-900">
                  {loading ? (
                    ...
                    items.map((item) => {
                      ...
                      const isAlreadyRequisitioned = existingIds.includes(
                        item.id
                      );

                      // Calculate remaining stock based on existing requisitions in this job
                      const relatedRequisitions = existingRequisitions.filter(
                        (r) => r.item?.id === item.id || r.itemId === item.id
                      );
                      const totalRequestedInJob = relatedRequisitions.reduce(
                        (sum, req) => sum + (req.qtyRequest || 0),
                        0
                      );
                      const displayQty = Math.max(
                        0,
                        (item.qtyOnHand || 0) - totalRequestedInJob
                      );
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 font-medium text-slate-900 dark:text-slate-100">
                            <div className="flex items-center gap-2">
                              ...
                              {isAlreadyRequisitioned && (
                                <Badge
                                  variant="secondary"
                                  className="text-[10px] px-1.5 h-5 bg-green-100 text-green-700 border-green-200"
                                >
                                  เบิกแล้ว
                                </Badge>
                              )}
                          ...
                          <td className="px-4 py-3 text-center">
                            <Badge
                              variant="outline"
                              className={`font-normal text-xs whitespace-nowrap ${item.type === "EQUIPMENT"
                          ...
                          >
                            {isSelected ? (
                              <Input
                                ...
                                // --- [แก้ไข] ส่ง displayQty เข้าไปเป็น parameter ที่ 3 ---
                                onChange={(e) =>
      ...
      <Dialog open={isConfirmDialogOpen} onOpenChange={setIsConfirmDialogOpen}>
        <DialogContent className="sm:max-w-md bg-white dark:bg-gray-900 z-[1060]">
          <DialogHeader>
          ...
          <div className="py-4 text-base text-gray-600 dark:text-gray-300">
            คุณต้องการเบิกอุปกรณ์จำนวน <strong>{selectedIds.length}</strong> รายการ ใช่หรือไม่?
          </div>
          <DialogFooter className="space-x-4 sm:gap-0">
            <Button variant="outline" onClick={() => setIsConfirmDialogOpen(false)}>
              ยกเลิก
            </Button>
            <Button onClick={handleFinalConfirm} className="bg-blue-600 hover:bg-blue-700 text-white">
              ยืนยัน
            </Button>
          </DialogFooter>
        </DialogContent>
