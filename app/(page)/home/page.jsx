...
export default function HomePage() {
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            const recentJobsRes = await apiClient.get("/work-orders?pageSize=5&sort=createdAt:desc");
            ...
            if (session?.role === "CEO" || session?.role === "ADMIN") {
                const [allJobsRes, inProgressRes, completedRes, itemsRes] = await Promise.all([
                ...
                const lowStockCount = items.filter(i => (i.qty || 0) < (i.minQty || 5)).length;
            ...
            <main className="p-4 md:p-6 space-y-8 max-w-7xl mx-auto">
                ...
                {isFullDashboard && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        {statCards.map((stat, index) => (
                            <Card key={index} className={`shadow-md hover:shadow-lg transition-all duration-200 ${stat.border} ${stat.bg}`}>
                                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                    <CardTitle className={`text-sm font-medium ${stat.text || "text-slate-600 dark:text-slate-300"}`}>
                                        {stat.title}
                                    </CardTitle>
                                ...
                                <CardContent>
                                    {loading ? (
                                        ...
                                        <>
                                            ...
                                            <p className={`text-xs mt-1 ${stat.text || "text-slate-500"}`}>
                                                {stat.description}
                                            </p>
                                        </>
                                    )}
                ...
                <div className={`grid grid-cols-1 lg:grid-cols-3 gap-8`}>
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        {/* Recent Jobs - Visible for ALL Roles now */}
                        <Card className="border shadow-sm overflow-hidden">
                            <CardHeader className="flex flex-row items-center justify-between bg-slate-50/50 dark:bg-slate-900/50 border-b px-6 py-4">
                                <div>
                                    <CardTitle className="text-lg font-semibold">งานล่าสุด</CardTitle>
                                    <CardDescription>รายการงานที่มีการอัปเดตล่าสุด 5 รายการ</CardDescription>
                                </div>
                            ...
                            <CardContent className="p-0">
                                {loading ? (
                                    ...
                                    <div className="divide-y divide-slate-100 dark:divide-slate-800">
                                        {recentJobs.map((job) => (
                                            <div key={job.id} className="flex items-center justify-between p-4 hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="flex items-center gap-2 text-xs text-slate-500 mt-1">
                                                            <span className="flex items-center">
                                                                ...
                                                                {job.customer?.companyName || job.customer?.firstName || "ไม่ระบุลูกค้า"}
                                                            </span>
                                                            <span>•</span>
                                                ...
                                                <div className="text-right text-xs text-slate-500 shrink-0">
                                                    <p>{job.createdAt ? format(new Date(job.createdAt), "d MMM yyyy", { locale: th }) : "-"}</p>
                                                    <p>{job.createdAt ? format(new Date(job.createdAt), "HH:mm") : "-"}</p>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                    ...
                    {/* Sidebar Area (Right 1/3) - Visible for ALL Roles now */}
                    <div className="space-y-8">
                        {/* Recent Activity Feed */}
                        <Card className="border shadow-sm h-full">
                            <CardHeader className="bg-slate-50/50 dark:bg-slate-900/50 border-b px-6 py-4">
                                ...
                                <div className="relative pl-6 border-l border-slate-200 dark:border-slate-800 space-y-8">
                                    {recentJobs.slice(0, 5).map((job, index) => (
                                        <div key={job.id || index} className="relative">
                                            <div className="absolute -left-[29px] top-1 h-3 w-3 rounded-full border-2 border-white dark:border-slate-950 bg-blue-500 ring-4 ring-slate-50 dark:ring-slate-950"></div>
                                            <div className="flex flex-col gap-1">
                                                <p className="text-sm font-medium text-slate-900 dark:text-slate-100">
                                                    มีการสร้างงานใหม่
                                                </p>
                                                <p className="text-xs text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-900 p-2 rounded-md">
                                                    {job.title || "งานใหม่"}
                                                </p>
                                                <span className="text-xs text-slate-400">
                                                    {job.createdAt ? format(new Date(job.createdAt), "d MMM HH:mm", { locale: th }) : "-"}
                                                </span>
                                            </div>
                                        </div>
                                    ))}
                                    {recentJobs.length === 0 && !loading && (
                                        <p className="text-sm text-slate-500">ไม่มีกิจกรรมล่าสุด</p>
                                    )}
                                </div>
                            </CardContent>
                        </Card>
...
function QuickActionCard({ href, icon, label, index }) {
    ...
    return (
        <Link href={href} className="group">
            <div className={`flex flex-col items-center justify-center p-4 rounded-xl transition-all duration-200 cursor-pointer bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md hover:-translate-y-1`}>
                <div className={`p-3 rounded-full bg-gradient-to-br ${gradient} text-white mb-3 group-hover:scale-110 transition-transform shadow-md`}>
                    {icon}
                </div>
...
function StatusBadge({ status }) {
    ...
    return (
        <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${styles[status] || "text-gray-600 bg-gray-100"}`}>
